...
;;; vector utilities ==========================================================
;; TODO: vectorとすると普通のベクタと間違えやすいので、二次元配列で表現されたベクタ、arrayed-vectorとして関数名を書き換える

;; ベクトルを配列形式で表現する. これによりmatrix-multiplyで行列と掛けたりできる
...
(defun ssum-vec (vec-lst)
  (saccumulate #'m+ (make-vector (length vec-lst) :initial-element 0.0d0) vec-lst))
...
(defun m-append-horizon (m1 m2)
  (let* ((m1-dims (array-dimensions m1))
	...
	(progn
	  (sfor (i 0 (1- (car m1-dims)))
	    (sfor (j 0 (1- (cadr m1-dims)))
	      (setf (aref product i j) (aref m1 i j)))
	    (sfor (j (cadr m1-dims) (1- (+ (cadr m1-dims) (cadr m2-dims))))
...
;;; Cholesky分解================================================================
;; ;; GSLLバージョンのCholesky行列
;; (asdf:oos 'asdf:load-op :gsll)
;; (defun cholesky-gsll (double-float-array)
;;   (gsll::letm ((M (gsll::matrix-double-float double-float-array)))
;;     (gsll::cholesky-decomp M)
;;     (gsll::data M)))

;; テスト用の正値対称行列
(set 'A (make-array '(3 3)
		    :initial-contents '((4d0 -1d0    1d0)
					(-1d0 4.25d0 2.75d0)
					(1d0  2.75d0 3.5d0))
		    :element-type 'double-float))

;; 正値対称行列AをCholesky分解し,下三角行列を返す
(defun cholesky (A)
  (let* ((dim (array-dimension A 0))
	 (L (make-array (list dim dim)
			:initial-element 0d0
			:element-type 'double-float)))
    ;; STEP1
    ...
    (sfor (i 1 (1- dim))
      ;; STEP4
      (setf (aref L i i)
	    (sqrt (- (aref A i i) (summation (k 0 (1- i))
				    (expt (aref L i k) 2)))))
      ;; STEP5
      (if (= i (1- dim))
	  L
